...
LOG_FUNCS = {
    ...
    'memory_scores': qa_em.compute_memory_score_subem,
    'query_length': qa_em.compute_search_query_length,
...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine, format_score=0., memory_score=0., reward_style='EM', query_step=1, log_path=None) -> None:
    ...
    def get_memory_subem(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            score = qa_em.compute_memory_score_subem(responses_str=responses_str, ground_truth=ground_truth)
    ...
    def __call__(self, data: DataProto , cur_step: int = -1 , total_step: int = -1):
        ...
        for i in range(len(data)):
            ...
            # swordli
            decay_step = self.query_step
            decay_theta = 0.5 * (np.cos(np.pi * cur_step / decay_step) + 1 )  # 1->0
            if cur_step >= decay_step:
                decay_theta = 0
            if decay_step > 10000:
                decay_theta = 1
            #print ("decay_step:" , decay_step , " decay_theta:" , decay_theta)
            score = qa_em.compute_dense_reward(solution_str=sequences_str, responses_str=responses_str, ground_truth=ground_truth, score_func=compute_score_fn, format_score=self.format_score, memory_score=self.memory_score, do_print_frac=1024, theta=decay_theta )
...
def main_task(config):
    ...
    memory_score = config.actor_rollout_ref.actor.memory_score
    format_score = config.actor_rollout_ref.actor.format_score
    query_step = config.actor_rollout_ref.actor.query_step
    reward_style = config.reward_model.reward_style
    #reward_type = config.reward_model.reward_type
    reward_fn = RewardManager(tokenizer=tokenizer, num_examine=config.reward_model.train_num_examine, log_path=train_log_jsonl, format_score=format_score, memory_score=memory_score, reward_style=reward_style, query_step=query_step)
